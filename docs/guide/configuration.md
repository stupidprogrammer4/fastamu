...
Settings are validated with Pydantic at application startup. `get_settings()` reads `config.yml` relative to the process working directory and caches the result. Editing the file does not reload a running process automatically.
