...
@router.get(
    "",
    response_model=APIResponse[JobsOverviewOut, None],
    response_model_exclude_defaults=True,
)
async def jobs_overview(
    service: FromDishka[IJobService],
) -> APIResponse[JobsOverviewOut, None]:
...
@router.get(
    "/running",
    response_model=APIResponse[RunningJobOut, None],
    response_model_exclude_defaults=True,
)
async def running_jobs(
    service: FromDishka[IJobService],
) -> APIResponse[RunningJobOut, None]:
...
@router.get(
    "/{task_id}",
    response_model=APIResponse[JobStatusOut, None],
    response_model_exclude_defaults=True,
)
async def job_status(
    task_id: str, service: FromDishka[IJobService]
) -> APIResponse[JobStatusOut, None]:
