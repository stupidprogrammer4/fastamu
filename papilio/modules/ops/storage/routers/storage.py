...
route guarded by AccessScope.STORAGE) plus the unauthenticated file-download
route."""
...
from papilio.modules.ops.storage.routers.schemas import MediaOut
from papilio.types.aliases import PageType, PerPageType

router = APIRouter(
    prefix="/storage", tags=["Storage"], route_class=DishkaRoute
)
_guarded = [Depends(require_access(AccessScope.STORAGE))]
